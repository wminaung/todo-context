...
export const Task = ({ children, todo }: TaskProps) => {
  ...
  const [{ overClass }, dropRef] = useDrop(
    () => ({
      accept: "todo",
      drop: (item: { id: string }) => doDropFun(item.id),
      collect: (monitor) => ({
  ...
  return (
    ...
    >
      ...
      <input
        ...
        } w-11/12   hover:cursor-grab active:cursor-grabbing  bg-inherit outline-none px-2  text-lg text-inherit placeholder:text-sm select-none`}
